...
$(function(){
	...
	//导出交易明细
	$.exportGetSplitFeeDetail = function (agencyId, dealTypeId,serialnumber,terminalId,startValue,endValue){		  
		
		var datestart = $('#datestart').val();
		var dateend = $('#dateend').val();
		if(datestart.substr( 0,6)!=dateend.substr(0,6)){
				 alert("查询日期的范围在一周之内，并且开始日期与结束日期必须在同一个月");
				 return  ;
		}
		...
		$.getToPost('${ctx}/splitfee/splitfee.do?method=exportGetSplitFeeDetailAll',{
			agencyId:agencyId,
			datestart:datestart,
			dateend:dateend,
	...
	$.viewSplitFeeDetail = function() {
		...
		$('#viewSplitFeeDetail').datagrid(
						...
						nowrap : true,
						fitColumns:false,
						url:"${ctx}/splitfee/splitfee.do?method=getSplitFeeDetailAll",
						queryParams : {								
							agencyId:agencyId,
							datestart:datestart,
							dateend:dateend,
						...
						columns : [ [
										...
										{field : "terminalCode",width :100,title : "终端号",align : "center"},
										{field : "dealTypeStr",width :80,title : "交易类型",align : "center"},
										{field : "dealData",width : 80,title : "交易日期",align : "center"},
										{field : "transAcount",width :100,title : "交易金额",align : "center"},
										{field : "feeAmt",width :100,title : "手续费金额",align : "center"},
									//	{field : "moneyLevel",width : 100,title : "分润金额 ",align : "center"},
									//	{field : "moneyLevelNext",width : 100,title : "下级分润金额 ",align : "center"}
						...
						toolbar:[{id:'btnadd',text:'导出',iconCls:'icon-undo',
							handler:function(){
								$.exportGetSplitFeeDetail(agencyId,dealTypeId,serialnumber,terminalId,startValue,endValue);
							}
						}],
						pagination : true,
						rownumbers : true,
...
<body id="indexd" bgcolor="#ff0000">
	...
	<tr>
		...
		<td>终端号：</td>
		<td><input type="text" name="terminalId" id="terminalId"/></td>
	</tr>
	<tr>
	<td>交易金额起始值：</td>
		<td>

...
$.openWin = function(obj) {
	...
	$('#dealTypeId').val('');
	$('#dealTypeNameU').val('');
	$('#dealTypeServCodeU').val('');
	$('#dealTypeTradCodeU').val('');
	...
	if (flag != "-1") {
		...
		$("#dealTypeServCodeU").val(row.serverCode);
		$("#dealTypeTradCodeU").val(row.tradeCode);
...
$.save = function() {
	...
	//14.10.31 新需求，交易类型新增时，加入服务编码和渠道编码
	var dealTypeServCode = $("#dealTypeServCodeU").val();
	var dealTypeTradCode = $("#dealTypeTradCodeU").val();
	...
	if ($.trim(dealTypeServCode) == "") {
		$.messager.alert("提示 ", "请输入服务编码");
		return false;
	}
	if ($.trim(dealTypeTradCode) == "") {
	...
	if (flag == "-1") {
		...
		$.post("${ctx}/dealtype/dealtype.do?method=addDealType", {
			...
			//14.10.31 新需求，交易类型新增时，加入服务编码和渠道编码
	        dealTypeServCode :dealTypeServCode,
	        dealTypeTradCode :dealTypeTradCode
		}, function(data) {
			...
			<div region="center" border="true"
				...
				<input type="hidden" id="dealTypeId" name="dealTypeId" />
					<table width="100%">
						...
						<tr>
							<td align="left">服务编码：</td>
							<td align="left"><input type="text" name="dealTypeServCodeU"
								id="dealTypeServCodeU" maxlength="100"/>
							</td>
							<td align="right">渠道编码：</td>
							<td align="left"><input type="text" name="dealTypeTradCodeU"
								id="dealTypeTradCodeU" maxlength="100"/>
							</td>
						</tr>
						<tr>
